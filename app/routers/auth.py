...
user_role = Depends(RoleChecker([UserRole.ADMIN, UserRole.CUSTOMER, UserRole.SERVICE_TECH, UserRole.SUPPLIER]))
...
async def login(user_data: LoginRequest, db: AsyncSession = Depends(get_db)):
    ...
    user = await auth_service.get_user(email, db)
...
async def create_user(
    ...
    message = await email_service.create_message(
        recipients=[user.email],
        subject="Verify your email",
        template_body={"user_name": user_name, "verification_link": email_verification_link}
    )
...
async def request_email_verification_link(
    ...
    user_name = f"{user.first_name} {user.last_name}"   # join user's first name and last name separated by a whitespace
...
@router.get('/verify-account/{user_private_key}')
async def verify_email(user_private_key: str, db: AsyncSession = Depends(get_db)):
    ...
    user = await auth_service.get_user(user_email, db)
    if not user:
        raise exceptions.UserNotFoundException()

    await auth_service.update_user_profile(user, {'is_verified': True}, db)
    return JSONResponse(
        content={
...
async def reset_password(user_email: ResetPasswordSchema, bg_task: BackgroundTasks, db: AsyncSession = Depends(get_db)):
    """
    ...
    email = user_email.email
    user = await auth_service.get_user(email, db)   # get user
    user_name = f"{user.first_name} {user.last_name}"   # join user's first name and last name separated by a space
...
async def confirm_reset_password(
    ...
    await auth_service.update_user_profile(user, {'password': user_hashed_password}, session)
    return JSONResponse(
        content={
